...
2. Run the CLI:
```bash
npx tsx ~/.claude/plugins/cognigy-claude-plugin/cli/src/index.ts list <resource>
```
...
3. Check the result:
   ...
   - **Exit 1** — error. Show the `error` field. Common fixes:
     - `No .env file found` → ask user to run the CLI init command first: `npx tsx ~/.claude/plugins/cognigy-claude-plugin/cli/src/index.ts init`
